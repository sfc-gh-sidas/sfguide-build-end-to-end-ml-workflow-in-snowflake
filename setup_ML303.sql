...
CREATE OR REPLACE WAREHOUSE E2E_MLOPS_WH
    ...
    INITIALLY_SUSPENDED = FALSE; -- Start in a suspended state to save costs
...
CREATE OR REPLACE WAREHOUSE E2E_ML_HOL_WAREHOUSE WITH
  ...
  AUTO_SUSPEND = 600 -- Consider a more appropriate auto-suspend time for ML workloads
  AUTO_RESUME = TRUE
...
FROM '@E2E_MLOPS_SUMMIT_PROD.DEFAULT_SCHEMA.GITHUB_REPO_E2E_SNOW_MLOPS/branches/goyetc_summit/' 
MAIN_FILE = 'ML303_SUMMIT_DEMO.ipynb' 
QUERY_WAREHOUSE = E2E_ML_HOL_WAREHOUSE
RUNTIME_NAME = 'SYSTEM$GPU_RUNTIME' 
COMPUTE_POOL = 'CP_GPU_NV_S_4'
IDLE_AUTO_SHUTDOWN_TIME_SECONDS = 3600;
